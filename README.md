...
- **多线程下载加速**：内置 Rust 编写的高效下载核心，支持 HTTP/HTTPS 多线程并发请求，充分压榨带宽。
- **HLS/M3U8 视频下载**：自动解析 m3u8 索引文件并并行下载分片，自动合并为完整视频。
- **BT/BitTorrent 下载**：支持 BT 种子解析、磁力链接解析及任务下载。
- **浏览器接管（Bridge）**：提供 Chrome/Edge MV3 扩展，在浏览器发起下载时自动拦截并交由桌面端处理。
- **网页视频流嗅探**：在 Twitter/X 以及 TikTok 页面自动嗅探流媒体，支持在网页端直接预览，并一键推送到桌面端进行多线程下载。
