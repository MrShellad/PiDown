...
pub struct ServerCapabilities {
    ...
    /// The final redirected URL after resolving HTTP redirects
...
/// Probe server capabilities with a HEAD request, falling back to GET Range: bytes=0-0
pub async fn probe_server(
    ...
    // If it was a GET request with Range, and it returned PARTIAL_CONTENT (206),
    // we must parse the total length and range support from the Content-Range header!
    if response.status() == reqwest::StatusCode::PARTIAL_CONTENT {
        supports_range = true;
        if let Some(content_range) = headers.get("content-range").and_then(|v| v.to_str().ok()) {
            if let Some((_, _, total_size)) = parse_content_range(content_range) {
